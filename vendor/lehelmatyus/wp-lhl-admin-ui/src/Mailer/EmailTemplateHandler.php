...
class EmailTemplateHandler {
    ...
    public function __construct($to, $subject = '', $body = '', $tokens = [], $is_html = false, $tamplate_path = '', $debug = false) {

        // must be first since body validation in set_body depends on it
        $this->set_is_html($is_html);
    ...
    public function send($template_path = "") {
    ...
    private function __validate_body($body) {
        ...
        if ($this->get_is_html()) {
            return $body = $this->__sanitize_html($body);
        }
